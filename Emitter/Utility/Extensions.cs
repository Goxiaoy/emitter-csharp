...
namespace System
{
    ...
    internal static class Utils
    {
        ...
        /// <returns></returns>
        public static bool TryParseUInt32(string str, NumberEncoding style, out uint result)
        {
            bool sign;
            ulong tmp;

            var bresult = Helper.TryParseUInt64Core(str, style == NumberEncoding.Hexadecimal ? true : false, out tmp,
                out sign);
            result = (uint)tmp;
        ...
        {
            long result;
            if (TryParseInt64(str, out result)) return result;
            throw new Exception();
        }

        public static long ParseInt64(string str, NumberEncoding style)
        {
            if (style == NumberEncoding.Hexadecimal) return ParseInt64Hex(str);
        ...
        {
            ulong result;
            if (TryParseInt64Hex(str, out result)) return (long)result;
            throw new Exception();
        }
        ...
        public static DateTime FromIso8601(string date)
        {
            ...
            var utc = date.EndsWith("Z");

            var parts = date.Split('T', 'Z', ':', '-', '.', '+');
            ...
            var dt = new DateTime(Convert.ToInt32(year), Convert.ToInt32(month), Convert.ToInt32(day),
        ...
        internal static class Helper
        {
            ...
            public static bool TryParseUInt64Core(string str, bool parseHex, out ulong result, out bool sign)
            {
                ...
                var len = str.Length;
                var posn = 0;
                while (posn < len && IsWhiteSpace(str[posn])) posn++;

                // Check for leading sign information.
                var nfi = CultureInfo.CurrentUICulture.NumberFormat;
                var posSign = nfi.PositiveSign;
                var negSign = nfi.NegativeSign;
                sign = false;
                while (posn < len)
                {
                    ch = str[posn];
